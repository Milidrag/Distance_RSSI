...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomDistanceHdle;                    /**< DistanceMeasure handle */
 ...
 */
#define COPY_DISTANCEMEASURE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x40,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for DistanceMeasure +
   *                                2 for My_LED_Char +
   *                              = 3
   */

  COPY_DISTANCEMEASURE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             3,
                             &(CustomContext.CustomDistanceHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: Distance, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: Distance \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomDistanceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeLed_C,
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_LED_C:
      ret = aci_gatt_update_char_value(CustomContext.CustomDistanceHdle,
                                       CustomContext.CustomLed_CHdle,
                                       0, /* charValOffset */
